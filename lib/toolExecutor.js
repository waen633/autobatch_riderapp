...
async function executeTool(toolName, args, baseUrl) {
  ...
  switch (toolName) {
    ...
    case 'get_riders': {
      ...
      // summary ป้องกัน AI นับ boolean เอง
      const ready   = riders.filter(r => r.ready_for_auto_assign === true);
      const onJob   = riders.filter(r => r.job_on_hand_id && r.no_active_job === false);
      const onBreak = riders.filter(r => r.not_on_break === false);
      return {
        summary: {
          total:        riders.length,
          readyCount:   ready.length,
          onJobCount:   onJob.length,
          onBreakCount: onBreak.length,
        },
        ready:   ready.map(r => ({ name: r.name, queue: r.queue })),
        onJob:   onJob.map(r => ({ name: r.name, job_on_hand_id: r.job_on_hand_id, job_on_hand_status: r.job_on_hand_status })),
        onBreak: onBreak.map(r => ({ name: r.name })),
        data: riders,
      };
    }
    ...
    case 'get_live_riders': {
      ...
      // จัดกลุ่มให้ AI อ่านตรงๆ ไม่ต้องนับ boolean เอง
      const ready    = riders.filter(r => r.eligible === true);
      const onJob    = riders.filter(r => r.eligible === false && r.jobId);
      const onBreak  = riders.filter(r => r.eligible === false && !r.jobId && r.inPool === true);
      const notInPool = riders.filter(r => r.inPool === false);
      return {
        summary: {
          total:          riders.length,
          readyCount:     ready.length,
          onJobCount:     onJob.length,
          onBreakCount:   onBreak.length,
          notInPoolCount: notInPool.length,
        },
        ready:     ready.map(r => ({ name: r.name, userId: r.userId })),
        onJob:     onJob.map(r => ({ name: r.name, userId: r.userId, jobId: r.jobId, jobStatus: r.jobStatus })),
        onBreak:   onBreak.map(r => ({ name: r.name, userId: r.userId })),
        notInPool: notInPool.map(r => ({ name: r.name, userId: r.userId })),
        stores: data.stores || [],
      };
    }
    ...
    case 'get_delivery_speed_trend': {
      const params = new URLSearchParams({ storeCode: args.storeCode, mode: 'daily' });
      if (args.days) params.set('days', args.days);
      const res  = await fetchAPI(`${baseUrl}/api/analytics/delivery-speed?${params}`);
      const rows = res.data || res; // รองรับทั้ง { mode, data } และ array เดิม
      const actual    = rows.filter(d => !d.predicted);
      const predicted = rows.find(d => d.predicted);
      return {
        trend: actual.map(d => ({
          date: d.date,
